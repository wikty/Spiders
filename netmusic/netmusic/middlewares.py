...
class ProxyPhantomjsRequestMiddleware(PhantomjsRequestMiddleware):
    ...
    def __init__(self, phantomjs_path=None, extra_script_file=None, proxy=None):
        super(ProxyPhantomjsRequestMiddleware, self).__init__(phantomjs_path, extra_script_file)
        if not proxy:
    ...
    def get_driver(self, **kwargs):
        if not self.driver:
            self.driver = webdriver.PhantomJS(executable_path=self.driver_path, proxy=proxy, **kwargs)
        return self.driver
    ...
    def from_crawler(cls, crawler):
        ...
        return cls(phantomjs_path, extra_script_file, proxy)


class ProxyFirefoxRequestMiddleware(BaseJsRequestMiddleware):

    def __init__(self, geckodriver_path=None, extra_script_file=None, proxy=None):
        super(ProxyFirefoxRequestMiddleware,self).__init__(geckodriver_path, extra_script_file)
        if not proxy:
            raise Exception('proxy setting is invalid')
    ...
    @classmethod
    def from_crawler(cls, crawler):
        geckodriver_path = crawler.settings.get('GECKODRIVER_PATH')
        proxy = crawler.settings.get('HTTP_PROXY')
        extra_script_file = crawler.settings.get('EXTRA_SCRIPT_FILE')

        return cls(geckodriver_path, extra_script_file, proxy)